...
//修改记录：无
//****************************************************************** 
void GUI_DrawFont56(u16 x, u16 y, u16 fc, u16 bc, u8 *s,u8 mode)
{
	u8 i,j;
	u16 k;
	u16 HZnum;
	u16 x0=x;
	HZnum=sizeof(tfont56)/sizeof(typFNT_GB56);	//自动统计汉字数目
	for (k=0;k<HZnum;k++) 
			{
			  if ((tfont56[k].Index[0]==*(s))&&(tfont56[k].Index[1]==*(s+1)))
			  { 	LCD_SetWindows(x,y,x+56-1,y+56-1);
				    for(i=0;i<56*7;i++)
				    {
						for(j=0;j<8;j++)
				    	{
							if(!mode) //非叠加方式
							{
								if(tfont56[k].Msk[i]&(0x80>>j))	LCD_DrawPoint_16Bit(fc);
								else LCD_DrawPoint_16Bit(bc);
							}
							else
							{
								POINT_COLOR=fc;
								if(tfont56[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y);//画一个点
								x++;
								if((x-x0)==56)
								{
									x=x0;
...
{					
    ...
    { 
        ...
        {   
            ...
            bHz=0;//有汉字库
      if(size==56)
      GUI_DrawFont56(x,y,fc,bc,str,mode);				
			else if(size==32)
